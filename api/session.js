...
// Appelé uniquement quand le visiteur ouvre le widget. Renvoie l'historique et le quota restant.
...
export async function GET(request) {
  try {
    ...
    if (!session) return error(429, "Trop de nouvelles conversations depuis votre connexion aujourd'hui.", { code: reason || 'too_many_sessions' });
    const rows = await store().loadRows(session.id);
    const headers = {};
    if (setCookie) headers['set-cookie'] = setCookie;
    return json({ messages: toDisplay(rows), remaining: remainingFor(session), limit: env.maxMessagesPerSession, status: session.status }, { headers });
  } catch (e) {

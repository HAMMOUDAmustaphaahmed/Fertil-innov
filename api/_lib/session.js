...
export const COOKIE_NAME = 'fi_sid';
...
export function cookieHeader(id) {
  const parts = [`${COOKIE_NAME}=${id}.${sign(id)}`, 'Path=/', 'HttpOnly', 'SameSite=Lax', `Max-Age=${COOKIE_MAX_AGE}`];
  if (env.isProd) parts.push('Secure');
  return parts.join('; ');
}

/** Retrouve la session du cookie, ou en crée une (si `create`, dans la limite du quota IP). */
export async function resolveSession(request, { create = false } = {}) {
  ...
  if (sessionsToday > env.maxSessionsPerIpPerDay) return { session: null, setCookie: null, created: false, reason: 'too_many_sessions' };
